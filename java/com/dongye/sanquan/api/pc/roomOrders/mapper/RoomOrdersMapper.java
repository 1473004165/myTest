...
public interface RoomOrdersMapper extends BaseMapper<RoomOrdersVo> {
    ...
    @Select("SELECT order_id,user_name,order_goal,ORDER_PEOPLE_NUM,o.ROOM_ID,ROOM_DORM,ROOM_NAME,ROOM_CAPACITY,ORDER_START_TIME,ORDER_END_TIME\n" +
            "/*where u.order_id = o.order_id and o.room_id = r.room_id*/\n" +
    ...
    List<RoomOrdersVo> selectCheckingOrderList(int orderStatus);
    ...
    List<RoomOrdersVo> selectDeniedOderList(int orderStatus);
    ...
    @Select("SELECT order_id,user_name,order_goal,ORDER_PEOPLE_NUM,o.ROOM_ID,ROOM_DORM,ROOM_NAME,ROOM_CAPACITY,ORDER_START_TIME,ORDER_END_TIME\n" +
            ...
            "from room_order o left join user u on u.user_id = o.user_id\n" +
            "left join room r on o.room_id = r.room_id\n" +
            "where order_status = 2 or order_status = 3 or order_status = 4;")
    @Results({
            @Result(column = "ORDER_ID", property = "orderId"),
            @Result(column = "USER_NAME", property = "userName"),
            @Result(column = "ORDER_GOAL", property = "orderGoal"),
    ...
    List<RoomOrdersVo> selectPassedOrderList(int orderStatus);
    ...
    @Update("UPDATE ROOM_ORDER SET ORDER_STATUS = 2 WHERE ORDER_ID = #{orderId};")
    int updatePassedRoomOrder(int orderId,int orderStatus);
    ...
    @Update("UPDATE ROOM_ORDER SET ORDER_STATUS = 6 WHERE ORDER_ID = #{orderId};")
    int updateDeniedRoomOrder(int orderId,int orderStatus);
